...
class TCPInteractions(GCPInteractions):
    def __init__(self, node_dims: ScalarVector, edge_dims: ScalarVector, cell_dims: ScalarVector, cfg: DictConfig, layer_cfg: DictConfig,
        ...
        ff_interaction_layers = [
            ff_TCP(
                (
                    cell_dims.scalar + node_dims.scalar * 3 + edge_dims.scalar,
                    cell_dims.vector + node_dims.vector * 3 + edge_dims.vector),
                hidden_dims,
                input_type=TCP.CELL_TYPE,
                nonlinearities=("none", "none")
                if layer_cfg.num_feedforward_layers == 1
        ...
        self.attentive_edge2sse = GeometryLocationAttention(
    ...
    def forward(
        ...
        edge_to_sse_mapping = ccc.incidence_matrix(1, 2)
        ...
        edge_rep_agg_inner = self.attentive_edge2sse(
            from_rank_sv=edge_message,
            to_rank_sv=hidden_residual_cell,
            incidence_matrix=edge_to_sse_mapping,
            from_frame=frames,
            to_frame=cell_frames,
            from_pos=edge_com,
            to_pos=sse_com[edge_to_sse_mapping.indices()[1]]
        )
        cell_edge_index, mask = TCP.get_sse_edge_index_and_mask(edge_index, node_to_sse_mapping)
        ...
        cell_hidden_residual = ScalarVector(*hidden_residual_cell.concat((cell_rep, node_rep_agg, edge_rep_agg, edge_rep_agg_inner)))  # c_i || h_i || e_i || m_e

...
from topotein.models.graph_encoders.layers.tcp_v0 import TCP
...
class TVPConv(MessagePassing):
    ...
    def __init__(
        ...
        super(TVPConv, self).__init__(aggr=aggr)
        self.si, self.vi = in_dims
        self.so, self.vo = out_dims
...
class TVPConvLayer(nn.Module):
    ...
    def __init__(
        ...
        super(TVPConvLayer, self).__init__()
        self.conv = TVPConv(
            node_dims,
            node_dims,
            edge_dims,
        ...
        if n_feedforward == 1:
            ff_func.append(
                GVP_(sse_dims, sse_dims, activations=(None, None))
            )
        else:
            hid_dims = 4 * sse_dims[0], 2 * sse_dims[1]
            ff_func.append(GVP_(sse_dims, hid_dims))
            ff_func.extend(
                GVP_(hid_dims, hid_dims) for _ in range(n_feedforward - 2)
            )
            ff_func.append(GVP_(hid_dims, sse_dims, activations=(None, None)))
    ...
    def forward(
        ...
        N0_2 = ccc.incidence_matrix(0, 2)
        cell_edge_index, mask = TCP.get_sse_edge_index_and_mask(edge_index, N0_2)

        if autoregressive_x is not None:
            raise NotImplementedError("autoregressive_x not implemented yet")

        else:
            dh = self.conv(x, edge_index, edge_attr)
            dh_sse = self.sse_conv(sse_attr, cell_edge_index, tuple_index(edge_attr, mask))
        ...
        sse_x = (
            self.sse_norm[0](tuple_sum(sse_attr, self.sse_dropout[0](dh_sse)))
            if self.residual
            else dh_sse
        )
        dh_sse = self.sse_ff_func(sse_x)
        sse_x = (
            self.sse_norm[1](tuple_sum(sse_x, self.sse_dropout[1](dh_sse)))
            if self.residual
            else dh_sse
        )

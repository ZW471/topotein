...
class TCPInteractions(GCPInteractions):
    def __init__(self, node_dims: ScalarVector, edge_dims: ScalarVector, sse_dims: ScalarVector, cfg: DictConfig, layer_cfg: DictConfig,
        ...
        ff_interaction_layers = [
            ff_TCP(
                (
                    sse_dims.scalar * 2 + node_dims.scalar,
                    sse_dims.vector * 2 + node_dims.vector),
                hidden_dims,
                nonlinearities=("none", "none")
                if layer_cfg.num_feedforward_layers == 1
                else cfg.nonlinearities,
    ...
    def forward(
        ...
        sse_hidden_residual = ScalarVector(*sse_hidden_residual.concat((sse_rep, node_rep_agg)))  # c_i || h_i || e_i || m_e

    ...
    def forward(self, batch: Union[Batch, ProteinBatch]) -> EncoderOutput:
        ...
        for layer in self.interaction_layers:
            (h, chi), batch.pos, (c, rho), (p, pi)  = layer(
                ...
                frames=batch.f_ij,
                sse_frames=batch.f_ij_cell,
                pr_frames=batch.pr_frames,
                node_frames=localize(batch, 0),
        ...
        if self.predict_node_pos:
            ...
            if self.predict_node_rep:
                ...
                batch.f_ij = self.localize(centralized_node_pos, 1)
                batch.f_ij_cell = self.localize(centralized_node_pos, 2)
                batch.pr_frames = self.localize(centralized_node_pos, 3)
